...
import { ChevronDownIcon, CloseIcon, CopyIcon, PlusIcon, TrashIcon, GithubIcon, ExportIcon, DragHandleIcon, LinkIcon } from './icons'
...
function isPristineNewAPIMartProfile(profile: ApiProfile) {
  const defaultProfile = createDefaultAPIMartProfile({ id: profile.id, name: '新配置' })
  return profile.name === '新配置' &&
    profile.provider === 'custom-dragoncode-gpt-image-2' &&
...
- $prompt：当前提示词。
- $params.size、$params.resolution（1k/2k/4k）、$params.output_format、$params.output_compression、$params.moderation、$params.n：应用内参数。
...
profiles 中不要包含 apiKey（应用统一从环境变量 NEXT_PUBLIC_DEFAULT_API_KEY 读取）。
...
## 同步接口示例
{"customProviders":[{"id":"custom-example-sync","name":"示例同步服务商","submit":{"path":"images/generations","method":"POST","contentType":"json","body":{"model":"$profile.model","prompt":"$prompt","size":"$params.size","resolution":"$params.resolution","output_format":"$params.output_format","moderation":"$params.moderation","output_compression":"$params.output_compression","n":"$params.n"},"result":{"imageUrlPaths":["data.*.url"],"b64JsonPaths":["data.*.b64_json"]}},"editSubmit":{"path":"images/edits","method":"POST","contentType":"multipart","body":{"model":"$profile.model","prompt":"$prompt","size":"$params.size","resolution":"$params.resolution","output_format":"$params.output_format","moderation":"$params.moderation","output_compression":"$params.output_compression","n":"$params.n"},"files":[{"field":"image[]","source":"inputImages","array":true},{"field":"mask","source":"mask"}],"result":{"imageUrlPaths":["data.*.url"],"b64JsonPaths":["data.*.b64_json"]}}}],"profiles":[{"name":"示例同步服务商","provider":"custom-example-sync","baseUrl":"https://api.example.com/v1","model":"example-model-v1","apiMode":"images"}]}
...
export default function SettingsModal() {
  ...
  const [exportGroupId, setExportGroupId] = useState('__all__')
  ...
  useEffect(() => {
    ...
    const displaySettings = reusedTaskApiProfileId && normalizedSettings.profiles.some((profile) => profile.id === reusedTaskApiProfileId)
  ...
  const commitSettings = (nextDraft: AppSettings) => {
    ...
    const fallbackProfile = createDefaultAPIMartProfile({ id: newId('apimart') })
  ...
  const createNewProfile = () => {
    ...
    const profile = createDefaultAPIMartProfile({ id: newId('apimart'), name: '新配置' })
    const nextDraft = normalizeSettings({ 
        ...draft, 
  ...
  const exportGroupOptions = [
    ...
    ...(draft.groups ?? []).map((group) => ({ label: group.name, value: group.id })),
  ]

  const getExportGroupPayload = () => {
    if (exportGroupId === '__all__') return { groupId: undefined, groupName: '全部分组' }
    if (exportGroupId === '__ungrouped__') return { groupId: null, groupName: '未分组' }
    const group = (draft.groups ?? []).find((item) => item.id === exportGroupId)
    return { groupId: exportGroupId, groupName: group?.name ?? '分组' }
  }
  ...
  const handleCustomProviderJsonPaste = async () => {
    ...
    try {
      ...
      if (imported.profiles.length > 0) {
        ...
        const shouldReplaceActiveProfile = !editingCustomProviderId && isPristineNewAPIMartProfile(activeProfile) && !importedProfileAlreadyExisted
              ...
              <div className="space-y-4">
                <div className="block">
                  <div className="mb-1 flex items-center justify-between gap-3">
                    <span className="block text-sm text-gray-600 dark:text-gray-300">界面主题</span>
                    <div className="w-32">
                      <Select
                  ...
                  <div data-selectable-text className="text-xs text-gray-500 dark:text-gray-500">
                    选择软件的显示主题，支持手动设定明亮、暗黑，或者跟随您的操作系统主题。
                  </div>
                </div>
                ...
                <div className="block">
                  <div className="mb-1 flex items-center justify-between gap-3">
                    <span className="block text-sm text-gray-600 dark:text-gray-300">请求超时 (秒)</span>
                    <input
                      value={timeoutInput}
                      ...
                      min={10}
                      max={3600}
                      className="w-32 rounded-xl border border-gray-200/70 bg-white/60 px-3 py-1.5 text-xs text-gray-700 outline-none transition focus:border-blue-300 dark:border-white/[0.08] dark:bg-white/[0.03] dark:text-gray-200 dark:focus:border-blue-500/50"
                    />
                  </div>
                  <div data-selectable-text className="text-xs text-gray-500 dark:text-gray-500">
                    控制图像请求和异步恢复等待时间，网络或代理较慢时可以适当调大。
                  </div>
                </div>
                <div className="block">
                  <div className="mb-1 flex items-center justify-between">
              ...
              <div className="space-y-4">
                <div className="rounded-2xl border border-gray-100 bg-white p-4 dark:border-white/[0.06] dark:bg-white/[0.02] space-y-4 shadow-sm">
                  ...
                  <div className="flex items-center justify-between space-x-10">
                    <span className="block text-xs font-medium text-gray-500 dark:text-gray-400 whitespace-nowrap">导出范围</span>
                    <Select
                      value={exportGroupId}
                      onChange={(value) => setExportGroupId(String(value))}
                  ...
                  <button
                    onClick={() => exportData(getExportGroupPayload())}
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="relative inline-flex">
                      ...
                      <ViewportTooltip visible={llmPromptTooltipVisible} className="w-56 whitespace-normal text-center">
                        生成完整的服务商和配置信息，包含模型和接口地址，API Key 由环境变量提供。
                      </ViewportTooltip>
                    </span>
                    <button
                      type="button"
          ...
          >
            ...
            >
              ...
              <div className="text-[13px] text-gray-500 dark:text-gray-400 mb-5 leading-relaxed">
                API Key 由环境变量提供，复制的导入 URL 只包含服务商、地址和模型配置。
              </div>
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => {
                    copyProfileImportUrl(copyImportUrlProfile, copyImportUrlOptions)
                  }}
